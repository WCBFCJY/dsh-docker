...
if [ ! -f /etc/nginx/nginx.conf ]; then
    ...
    AUTH_CONF=""
    AUTH_INTERNAL_LOCATIONS=""
    if [ -f /etc/nginx/.htpasswd ]; then
        AUTH_CONF='auth_basic "dsh"; auth_basic_user_file /etc/nginx/.htpasswd; error_page 401 = @auth_failed;'
        AUTH_INTERNAL_LOCATIONS='location @auth_failed {
            set $challenge "";
            if ($http_authorization = "") {
                set $challenge "Basic realm=\"dsh\"";
                return 401;
            }
            add_header WWW-Authenticate $challenge always;
            return 444;
        }'
    fi
